...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        #Create numpy array of XX and sample randomly with replacement
        xx = np.array(XX, dtype = object)
        bs = xx[np.random.randint(len(xx), size=n),:]
        samples = bs[:, :bs.shape[1]-1].tolist()
        labels = bs[:, bs.shape[1]-1].tolist()
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                #If out-of-bag sample, make a classification from the first tree
